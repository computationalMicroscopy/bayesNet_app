    ...
    with st.spinner(f'Führe {num_samples} Simulationen durch...'):
        ...
        if total_samples > 0:
            ...
            if total_samples > 0:
                ...
                # Berechne die bedingte Wahrscheinlichkeit für aggressives Verhalten bei hohem Gefahrenpotenzial
                high_gefahr_samples = [s for s in sampled_data if s['gefahrenpotential'] == 'gefahrhoch']
                if high_gefahr_samples:
                    p_aggressiv_bei_hoch = sum(1 for s in high_gefahr_samples if s['aggressives_verhalten'] == 'aggressivja') / len(high_gefahr_samples)
                    st.subheader('Wahrscheinlichkeit für aggressives Verhalten:')
                    st.markdown(f"Die Wahrscheinlichkeit, dass die Person aggressives Verhalten zeigt, wenn das Gefahrenpotenzial als hoch eingeschätzt wird, beträgt: **{p_aggressiv_bei_hoch:.2%}**.")
                else:
                    st.info('Es wurden keine Fälle mit hohem Gefahrenpotenzial in der Stichprobe gefunden, um diese bedingte Wahrscheinlichkeit zu berechnen.')
                ...
                value_name_map = {
                    ...
                    'aggressivja': '', 'aggressivnein': 'nein', # "ja" entfernt
                    'sozialisoliertja': '', 'sozialisoliertnein': 'nein', # "ja" entfernt
                    'leistungsabfallja': '', 'leistungsabfallnein': 'nein', # "ja" entfernt
                    'warnsignaleja': '', 'warnsignalenein': 'nein', # "ja" entfernt
                    'vorherigefaelleja': '', 'vorherigefaellenein': 'nein' # "ja" entfernt
                }
                ...
                for node in ['familiaeres_uUmfeld', 'psychische_gesundheit', 'schulische_unterstuetzung',
                    ...
                    for internal_value in unique_internal_values:
                        ...
                        conditional_probs[f"{display_node_name} {display_value_name}"] = prob # "ja" entfernt
                ...
                profile_data_named = {
                    ...
                    'Gefahrenpotential': probabilities
                }
