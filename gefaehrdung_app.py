...
familiaeres_uUmfeld = {
    'stabil': p_familiaeres_stabil,
    'instabil': 1.0 - p_familiaeres_stabil
}

psychische_gesundheit = {
    'unauffällig': p_psychische_unauffaellig,
    'auffällig': 1.0 - p_psychische_unauffaellig
}

schulische_unterstuetzung = {
    'vorhanden': p_schulische_unterstuetzung_vorhanden,
    'mangelhaft': 1.0 - p_schulische_unterstuetzung_vorhanden
}

# Deine bestehenden Definitionen der bedingten Wahrscheinlichkeiten (aggressives_verhalten, soziale_isolation, etc.) bleiben gleich
...
def forward_sampling(noSamples):
  ...
  for i in range(noSamples):
    aktSample = []
    # Sampling der A-priori-Knoten mit den benutzerdefinierten Wahrscheinlichkeiten
    rnd = r.random()
    aktSample.append('stabil' if rnd <= familiaeres_uUmfeld['stabil'] else 'instabil')

    rnd = r.random()
    aktSample.append('unauffällig' if rnd < psychische_gesundheit['unauffällig'] else 'auffällig')

    rnd = r.random()
    aktSample.append('vorhanden' if rnd < schulische_unterstuetzung['vorhanden'] else 'mangelhaft')
